...
export default function AddAdminScreen({ navigation }) {
  useEffect(() => {
    console.log('AddAdminScreen mounted successfully');
  }, []);
  ...
  const handleAddAdmin = async () => {
    ...
    try {
      ...
      if (response.status == 200 || response.status == 201 || response.statusText == 'Created') {
        Alert.alert('Success', 'Admin added successfully! Status: Approved', [
          { text: 'OK' },
        ]);
      ...
      setLoading(false);
      Alert.alert('Error', error.response?.data?.message || 'Failed to add admin. Please try again.');
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.formContainer}>
        ...
        <Text style={styles.subtitle}>Register admin from Admin Center</Text>
        ...
        <View style={styles.statusContainer}>
          ...
          <Text style={styles.statusNote}>Admin status is automatically set to Approved</Text>
        </View>
