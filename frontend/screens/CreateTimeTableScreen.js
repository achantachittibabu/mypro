...
const CreateTimeTableScreen = ({ navigation }) => {
  ...
  const [morningFrom, setMorningFrom] = useState('09:00');
  const [morningTo, setMorningTo] = useState('12:00');
  const [morningPeriods, setMorningPeriods] = useState([]);
  const [lunchFrom, setLunchFrom] = useState('12:00');
  const [lunchTo, setLunchTo] = useState('12:45');
  const [afternoonFrom, setAfternoonFrom] = useState('12:45');
  const [afternoonTo, setAfternoonTo] = useState('15:30');
  const [afternoonPeriods, setAfternoonPeriods] = useState([]);
  ...
  const handleSaveTimeTable = () => {
    ...
    Alert.alert(
      'Success',
      `Timetable created successfully!${timetableType === 'student' ? ` for ${selectedClass}` : ''}`,
      [
        {
          text: 'OK',
      ...
      <Card style={styles.headerCard}>
        <Card.Content>
          <View style={styles.headerContent}>
            <View>
              <Title style={styles.headerTitle}>Create Time Table</Title>
              <Text style={styles.headerSubtitle}>Set up school timetable</Text>
            </View>
